<script>
	...
	function getAPYCAP(_capPool, _poolStats) {
		...
		let capUSDSupply = 400 * _capPool.supply; // assuming BCP price of 400
...
<div class='pools'>
    ...
    <div class='pool cap-pool' class:loading={poolIsLoading['cap'] || !$capPool.supply}>
    	...
    	<div class='description'>
    		Stake BCP to receive a share of revenue. <a href='#/buy'>Buy BCP</a>
    	</div>
    		...
    		<div class='row'>
    			<div class='label'>
    				...
    				{#if $allowances['cap'] && $allowances['cap']['capPool'] * 1 == 0}
    					<a on:click={() => {_approveCurrency('cap')}}>Approve BCP</a>
    				{:else}
    			...
    			<div class='value'>
    				{formatToDisplay($capPool.userBalance)} BCP <span class='grayed'>({formatToDisplay($capPool.supply*1 == 0 ? 0 : 100*$capPool.userBalance/$capPool.supply)}%)</span>
    			</div>
    		</div>
